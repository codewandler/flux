...
| `jq` | `path` (field/index path string), `input` (`var`/`lit`/`obj`/`list` node) — native `value.items[0]` lowers the index to `.items.0`; `value.headers["content-type"]` lowers the quoted object key to `.headers["content-type"]` |
