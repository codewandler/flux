...
<!-- Derived from the generated node-kinds table in crates/flux-lang/docs/reference.md
     (UPDATE=1 cargo test -p flux-lang --test skill_in_sync regenerates the source of truth);
     descriptions are lightly condensed for the public site — re-sync after node changes. -->
...
| `expr` | Pure inline computation. `formula` is a safe whitelist expression over named variables: arithmetic (`+ - * /`, `round(x,n)`, `abs`, `min(a,b)`, `max(a,b)`), comparison (`== != < <= > >=`), boolean (`&& \|\| !`, `true`/`false`), string functions (`len/lower/upper/trim/replace/repeat/reverse/contains/concat`), and string literals (`'…'`/`"…"`). `+` adds when both sides are numeric and concatenates otherwise. Because it yields a bool, an `expr` is also a valid `when`/`unless`/`until`/`assert` condition. `vars` maps variable names to node expressions (only `Lit` and `Var` are valid). No IO, no approval gate. |
| `fmt` | Pure string interpolation. `template` is a string with `{name}` placeholders substituted from already-bound session symbols (same `{name}`/`{{name}}` syntax as `Lit` interpolation). No IO, no approval gate. |
| `jq` | Pure JSON path extraction. `path` is a dot-path string (e.g. `".bitcoin.usd"` or `"results[0].value"`) applied to the JSON content of `input` (a `Var` or `Lit` node). No IO, no approval gate. |
| `parse` | Pure type coercion. Converts the string result of a `jq` or `fmt` node into a typed value. `as_type` is one of `"f64"`, `"i64"`, `"bool"`, `"json"`, `"string"`. No IO, no approval gate. |
...
| `ctx_append` | Accrete more symbols into an existing context pack (the `+=` marker). Immutably rebinds `ctx` to a *new* `Ctx` value (preserving the audit chain) with `add` appended, then re-applies the pack's budget. Pure. |
| `match` | Multi-way **exhaustive** branch: evaluate `subject` (a literal or bound symbol), then run the body of the first `case` whose `value` equals it — by JSON equality, so a *string* subject does not equal a *numeric* literal. If none match, run `default`. To branch on an op's result, bind it first or use `route`. The analyzer requires at least one case; at runtime an unmatched subject with no `default` is an error. |
| `route` | Model-routed branch — the signature *bounded non-determinism* primitive. Run `selector` (typically a model-backed op) to produce a label, then run the `case` whose `label` it names. The cases are fixed and analyzer-validated: the model chooses *which* declared branch runs, never *what*. Falls back to `default` when the label matches no case (an error if `default` is empty). |
| `fallback` | Ordered "first that succeeds wins" selector: run each branch in turn; the first that completes without error and yields a non-empty result wins. On a branch error (or empty result) the next is tried. If every branch errors, the last error propagates. `bind` names the winning result. |
| `timeout` | Bound the wall-clock of a sub-flow: run `body` with a `ms` deadline. If it does not finish in time the node errors (an enclosing `try`/`retry` may catch it). `bind` names the body's result. |
| `budget` | Cap the cost of a scope: run `body` but allow at most `limit` op dispatches within it (checked at statement boundaries). v1 counts dispatches. `bind` names the body's result. |
| `cap_scope` | **Capability scope**: run `body`, but restrict op dispatch to the tool names in `tools` — a call to anything outside that allowlist fails closed at the runtime's dispatch gate, even when the outer session policy would allow it. Capabilities only narrow on descent: a nested scope is intersected with its parent. Native text: `with_tools ["read", "grep"]` + an indented body. |
| `scope` | RAII-style **acquire → use → release** with guaranteed cleanup. Optionally run `acquire` first (binding its result to `bind`), then run `body`; `finally` **always** runs afterward — on normal completion, an early `return`, or an error. A `finally` failure surfaces only when the body itself succeeded. If `acquire` errors, `finally` does not run. |
| `saga` | Saga / **compensating transaction**: run each `step` in order; after a step's `body` succeeds, its `undo` is registered. If a *later* step fails, the runtime unwinds the registered `undo` bodies in **reverse** order (best-effort), then propagates the original error. A `return` inside a step is a successful early exit and does not compensate. |
| `once` | **At-most-once side effect** across re-runs — an effect-level `memo`. `label` is an explicit idempotency key: the first success is recorded durably; later re-runs in the same session reuse the stored result. A failed body records nothing and is retried. Requires a non-empty literal label. |
| `checkpoint` | **Durable resume point** for long-running / resumable flows. Top-level-only marker: a later re-run of the *same* flow in the *same* session fast-forwards past the already-completed prefix and continues from here. Requires a non-empty literal label. |
| `obj` | Build an **object value** from sub-expressions — the record constructor `{ k: expr, … }`. Pure; leaves must be pure value nodes (`var`/`lit`/`jq`/`expr`/`fmt`/`obj`/`list`); a call or control-flow leaf is rejected by the analyzer. |
| `list` | Build a **list value** from sub-expressions — the list constructor `[ expr, … ]`. Pure, same leaf rules as `obj`. |
