...
| `FLUX_APPROVAL_TIMEOUT_SECS` | How long a served agent under `--remote-approval` waits for a human decision at `/approvals` before **denying** the effect (default `120`). There is deliberately no "wait forever" value — an unbounded wait is a wedged turn, not a decision — and an unparseable value falls back to the default. |
