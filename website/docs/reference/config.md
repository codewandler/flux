...
| `FLUX_EXCHANGE_SERVICE_ACCOUNT_TOKEN` | Bearer token for one Exchange Service Account. It is registered for redaction at startup and is never accepted in argv, config files, URLs, or tool input. If either Exchange variable is absent, Exchange operations stay disabled while core Flux remains available. |
