...
| --- | --- | --- |
| `advanced-code-review.flux` | Full AI code-review pipeline: parallel context gather with `fallback`, budgeted `ctx` pack, `retry`/`timeout`, cognition ops (`ai.extract`/`ai.rank`/`ai.judge`/`ai.reason`, `synth`), model-routed `route`, and `@json` escapes for `confirm`, `saga`+undo, and `verify` | `flux flow run examples/advanced-code-review.flux -m <model>` — needs a model **and** the `flux-plugin-slack` plugin (`slack.message.send`); CI validates it parse-only. Its header's `flux run … --param` line is stale: the CLI has no flag to supply its declared flow parameters |
