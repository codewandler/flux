...
| `commit.flux` | Focused commit workflow: validates a conventional title and bulleted body, refuses a pre-populated index, stages explicit paths only, exposes the staged patch for approval, and commits without pushing | `flux flow run examples/commit.flux --inputs '{"paths":["path/to/file"],"title":"feat(scope): describe change","body":"- Explain what changed and why."}'` — modifies the Git index and creates a local commit after approval |
...
| `review.flux` | Project-adaptive review: classify the guarded file inventory and Git history, derive exactly four structured dimensions from only that classification, run four read-only reviewers in parallel, join their reports, and synthesize a final verdict | CLI: `flux flow run examples/review.flux -m <model> --yes`; harness: `flow_run({path: "examples/review.flux"})`. The harness rereads the file and current operation catalog for each call; needs a model and reviews the current checkout |
