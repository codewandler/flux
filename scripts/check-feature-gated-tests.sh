...
flux-channels	room-media	run	the D-208 media-sidecar suite (11 tests in tests/room_media.rs + 12 unit tests under rooms::media) plus D-232's browser-harness suite (tests/room_media_harness.rs); off by default and nothing in the workspace enables it, so `cargo test --workspace` compiles none of it. D-232's live-browser tests are `#[ignore]`d — CI has no browser and no network; the commands to run them by hand are in their doc comments and in the story. D-232's non-ignored legs shell out to `node` (the language the sidecar is written in) and **fail loudly if it is absent** rather than skipping: libtest hides a passing test's stderr, so an early `return` made them report `ok` having asserted nothing. `ubuntu-latest` ships Node, but this row does not depend on that staying true
