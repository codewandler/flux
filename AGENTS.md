...
- ⚠ **A regenerating run is RED on purpose, and only the exact value `1` arms it** (C-326). Every checked-in-golden guard — the two above, `crates/flux-plugin-protocol/tests/wire_contract.rs`, and `crates/flux-tui/tests/loop_mocks.rs` (A-144's mock snapshot set) — reads `FLUX_UPDATE_GOLDEN`, writes its file, then **fails** with `REGENERATED <path>`: a run that wrote a golden verified nothing and must not be reportable as a passing check. Review the diff, then re-run with the variable unset to actually verify. The old spelling was `UPDATE` gated on *presence*, so an ambient `UPDATE=0` — or an empty `UPDATE=` — silently rewrote all the goldens and printed `ok`. Never gate test behaviour on `env::var(..).is_ok()`; match on the value — and **refuse an unrecognized value** rather than falling back to checking, which would hand back a green run the author reads as "regenerated". `crates/flux-lang/tests/support/golden_mode.rs` is the canonical statement of all three rules; crates that cannot depend on it mirror the semantics.
