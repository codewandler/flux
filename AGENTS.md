...
**The node-kind tables are a single source of truth and are auto-generated — do not hand-edit them.** The `Node` enum's doc-comments in `crates/flux-lang/src/ast.rs` flow through `flux_lang::schema::node_kind_catalog()` into (a) the `emit_plan` planner prompt, (b) the "Node kinds at a glance" table in `crates/flux-lang/docs/reference.md`, (c) the `## Node kinds` table in the **flux-lang** language skill (`crates/flux-lang/skill/SKILL.md`), (d) the same table in the **flux-flow** engine skill (`.flux/skills/flux-flow/SKILL.md`), and (e) the public website's `website/docs/language/node-reference.md`. The **artifact-type prelude** has a parallel SSOT: the `flux_lang::prelude` struct doc-comments flow through `prelude_type_catalog()` into the `<!-- BEGIN/END generated:prelude-types -->` block in `reference.md`, the skill, and the website's `website/docs/language/types-and-effects.md` — regenerated by the same `skill_in_sync`/`website_in_sync` tests, so add a `Named` prelude type then `UPDATE=1 cargo test -p codewandler-flux-lang --test skill_in_sync` and `UPDATE=1 cargo test -p codewandler-flux-lang --test website_in_sync`. The generated blocks are fenced by `<!-- BEGIN/END generated:node-kinds -->`; three tests fail on drift: `cargo test -p codewandler-flux-lang --test skill_in_sync` (the language skill + reference), `cargo test -p codewandler-flux-flow --test skill_docs_in_sync` (the engine skill), and `cargo test -p codewandler-flux-lang --test website_in_sync` (the website pages — cell content there additionally escapes literal `|` for Docusaurus's table renderer). After adding/renaming a node kind or editing a variant doc-comment, regenerate with `UPDATE=1` on all three: `UPDATE=1 cargo test -p codewandler-flux-lang --test skill_in_sync`, `UPDATE=1 cargo test -p codewandler-flux-flow --test skill_docs_in_sync`, and `UPDATE=1 cargo test -p codewandler-flux-lang --test website_in_sync`.
