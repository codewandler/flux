...
impl Tool for FleetIsolateTool {
    ...
    async fn execute(&self, ctx: &ToolContext, params: Value) -> Result<ToolResult> {
        ...
        // Preflight 4: a clean base. `git worktree add <path> <head>` checks out HEAD, so
        // uncommitted work in the caller's checkout would silently NOT be in the worker's copy —
        // the worker would build on a base the coordinator cannot see.
        let (ok, status) = run_git(&system, &["status", "--porcelain"]).await?;
        if !ok {
            return Ok(ToolResult::error(format!(
                "fleet.isolate: git status failed: {status}"
            )));
        }
        if !status.is_empty() {
            // Same wording care as `git_revert`: `--porcelain` also lists untracked (`??`) entries,
            // which a plain `git stash` leaves exactly where they are.
            return Ok(ToolResult::error(format!(
                "fleet.isolate: an isolated checkout is made from HEAD, and this checkout is not \
                 clean — the worker would not see the uncommitted work below. Commit or stash the \
                 tracked changes; move, remove or `git stash -u` any untracked (`??`) \
                 entries:\n{status}"
            )));
        }
