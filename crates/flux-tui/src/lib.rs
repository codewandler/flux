...
/// The `/help` body: built-in keybindings/commands, plus (D-186) any discovered command files.
fn help_text(file_commands: &[flux_runtime::metadata::CommandFile]) -> String {
...
async fn handle_command(
    ...
    match name {
        "" | "help" => state.push(Entry::Notice {
            text: help_text(&state.file_commands),
            sev: Sev::Info,
        }),
...
mod tests {
    ...
    /// D-186: `/help` lists discovered command files (name, description, argument-hint) alongside
    /// the built-in keybinding summary.
    #[test]
    fn help_text_lists_discovered_command_files() {
        let file_commands = vec![test_command_file("review", "Review a PR", "<pr-number>")];
        let text = help_text(&file_commands);
        assert!(text.contains(HELP_TEXT));
        assert!(text.contains("/review"));
        assert!(text.contains("Review a PR"));
        assert!(text.contains("<pr-number>"));
    }

    #[test]
    fn help_text_without_command_files_is_unchanged() {
