...
use flux_tui::loopmock::{self, Viewport, LOAD_CASES, MOCKS};
...
fn explore(term: &mut Terminal<ratatui::backend::CrosstermBackend<io::Stdout>>) -> io::Result<()> {
    ...
    loop {
        ...
        match key.code {
            ...
            KeyCode::Char(c @ '1'..='5') => pick.mock = c as usize - '1' as usize,
...
fn draw(frame: &mut ratatui::Frame, pick: &Pick, theme: &Theme) {
    ...
    let keys = Line::from(vec![
        ...
        Span::styled(" recommendation  ", theme.muted_style()),
    ...
    let render = loopmock::render(mock, case, pick.vp, theme);
    let title = format!(
        " {} · {} · {}×{} ",
        spec.name,
        case.name(),
