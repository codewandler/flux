...
pub(super) async fn resolve_named_host(
    ...
    match backend_under_floor(host.backend, floor) {
        ...
        // C-683: ssh is the bootstrap, never the substrate. It does not share the arm below,
        // because unlike `microvm` it has no endpoint until one is *made*: the link has to be
        // bootstrapped before the identical client can be pointed at it. What resolves is still the
        // delivered remote system — same TLS, same bearer token, same negotiated version, same
        // handshake — reached through a forward this session holds for exactly as long as it holds
        // the substrate. Selection is also the act that carries the intent to *start* a far-side
        // serve; a probe only attaches.
        HostBackend::Ssh => {
