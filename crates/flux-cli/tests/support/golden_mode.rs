//! How a checked-in-golden guard decides between *checking* and *regenerating* — the one place
//! that reads the arming variable, shared by `skill_in_sync.rs` and `website_in_sync.rs`.
//!
...
//!    name an unrelated tool or a shell profile can export, so on such a machine every guard here
//!    overwrote its golden with whatever the code currently emitted and reported **ok** having
//!    compared nothing. The variable is now `FLUX_UPDATE_GOLDEN` (crate-scoped, like every other
//!    behaviour-changing variable in this workspace) and only the exact value `1` arms it.
...
//!    [`rewrote`].
//!
//! A copy of this module lives at `crates/flux-plugin-protocol/tests/support/golden_mode.rs`: that
//! crate ships on the independent plugin-protocol version line (C-143) and must not gain a
//! dependency on this one. Keep the two in step; each is covered by its own `golden_arming` test.

#![allow(dead_code)] // each including test target uses a different subset
