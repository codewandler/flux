...
Durable de-duplication after completion is recorded (an effect-level `memo`). The explicit `label`
contributes to the durable identity: after the body runs **to success** and the runtime appends its
`OnceCompleted` record, later re-runs skip that body and reuse the stored value. A crash after an
external effect succeeds but before the record is appended can repeat the effect on re-run, so use
external idempotency or a transaction when duplicates are unacceptable. A failed body records
nothing and is retried. `bind` optionally names the result.
...
`--resume`): `checkpoint` is the *cross-run caching*
