...
| 1, 8 | Compact **text parser** (text → AST) | ✅ | `src/parse.rs` + `src/format.rs`; `parse(format(ast)) == ast` — native spellings for **every node kind** (the 16-node coverage gap closed in L-60..L-63); `@json` remains only for unspellable shapes (non-identifier names (L-18), non-invertible `expr`, bracket-path `jq`); property-tested (`tests/roundtrip_property.rs`) |
