...
| 1, 8 | Compact **text parser** (text → AST) | ✅ | sole accepting lossless CST: `src/lexer.rs` + `src/parser.rs` → structured `src/cst_decode.rs` lowering + strict `src/lower_cst.rs`/`src/parse.rs`; tolerant ERROR recovery powers editors while compile APIs refuse malformed input. The logical-line parser is gone; frozen pre-cutover source/AST and error-class fixtures pin compatibility without executable legacy parser code. `parse(format(ast)) == ast`; native spellings for **every node kind** (L-60..L-63); `@json` remains only for unspellable shapes; corpus/property/range tests cover the cutover (`tests/cst_agreement.rs`, `tests/roundtrip_property.rs`, `tests/analyzer_ranges.rs`) |
