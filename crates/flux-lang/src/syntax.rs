...
pub enum SyntaxKind {
    ...
    REPEAT_STMT,     // repeat n [, until: cond] [-> $c] …
    UNTIL_CLAUSE,    // until cond  (legacy first body line of repeat/loop)
    ...
    AWAIT_STMT,      // await [$b =] "source" [, when: cond]
    CONFIRM_STMT,    // confirm "msg" [, risk: r] …
    THROTTLE_STMT,   // throttle "name", max: <max>, per: <window> …
    DEBOUNCE_STMT,   // debounce "name", wait: <ms> …
    ...
    /// One `, name: value` option on a canonical control header (L-96) — `, risk: medium`,
    /// `, backoff: exponential`, `, until: $done`. The leading comma belongs to the node, so the
    /// text either side of the option run is exactly the legacy space-keyword header.
    HEADER_OPTION,
