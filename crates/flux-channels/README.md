...
| `slack` | `bot_token secret "SLACK_BOT_TOKEN"` · `app_token secret "SLACK_APP_TOKEN"` · `allow_users [...]` · `allow_channels [...]` | `{ text, user, channel, thread, conversation }` |
...
- **slack** is compiled in by default (drop it with `--no-default-features`). It uses socket mode; posts each
  journey's result back to the originating thread; `allow_users`/`allow_channels` (empty = allow all) gate access.
  See the [Slack channel setup guide](../../website/docs/agent/slack-channel.md) for creating the app + tokens.
