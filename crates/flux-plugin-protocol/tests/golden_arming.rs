...
//!
//! `wire_contract` rewrites the pinned wire goldens when armed. The bug this pins shut was that
//! arming tested **presence, not value**: `std::env::var("UPDATE").is_ok()` meant `UPDATE=0`, and
//! even an empty `UPDATE=`, re-recorded the wire surface and let the guard report `ok` having
//! compared nothing — silently blessing exactly the change every plugin binary in the wild would
//! break on. So the assertions below are about *values*, and about the fact that the old, ambient,
...
//! `wire_contract` would race the guards running beside it on other threads.
