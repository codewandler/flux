...
- **`flow_run(name | path | inline_program, inputs?)`** accepts exactly one stored-flow name,
  confined workspace-relative `.flux` path, or Flux source supplied directly in the request. Path
  source is reread on each call; inline source is parsed without filesystem IO. It seeds `inputs` as
  prepended literal binds and runs in the current session by re-entering the depth-guarded `run_plan`
  path (`ctx.loop_host`), so lowering sees the live operation catalog and execution inherits the
  provider, session, and approval/IO envelope. Its compatibility-lenient input semantics are
  unchanged. The result includes a route receipt with an optional resolved path, flow name, and
  seeded input keys. (It needs a `LoopHost`, like `run_plan`.)
