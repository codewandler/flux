...
## Acceptance
- [ ] New module `crates/flux-tools/src/transform.rs`. `flux-tools/Cargo.toml` adds
      `flux-lang.workspace = true`.
- [ ] `map({items, path|expr, vars?})` — `path` plucks a dotted field per element
...
- [ ] `filter` extended: accepts `where` + optional `vars`, mutually exclusive with the
...
- [ ] `flatten({items, depth?})` — flatten `depth` levels (default 1, cap 8; higher = 400).
...
- [ ] All new ops registered in `flux-tools/src/lib.rs::register_cognition` and the
      `cognition` group in `flux-tools/src/groups.rs`; group description updated.
- [ ] Predicate error message special-cases leading `.` and `$` with rewrite hints
