...
- [x] `AgentSpec` owns an optional typed effort setting and passes it into `FlowEngine` without
      breaking existing constructors/builders.
- [x] Planner/repair calls, the completion fast path, compaction, and `CognitionPack` requests carry
      the same effort; sub-agents inherit it unless their role explicitly overrides it.
- [x] Provider codecs continue applying their existing capability gates/mappings. Unsupported models
      omit or reject the setting honestly; Flux does not silently claim it was applied.
- [x] `--effort` is visible in CLI help and provider docs no longer call it a no-op. Decide and
      document whether legacy `--think` is an alias or removed.
- [x] Capture-provider tests assert the request setting on every call class above.
- [x] Live OpenRouter or Codex low-vs-high probes record request usage, reasoning tokens, latency,
