...
## Acceptance
- [x] New public `crates/flux-lang/src/expr.rs` module carrying `Tok`, `tokenize_expr`,
...
- [x] `runtime.rs` and `analyze.rs` re-import from the new module; **all existing
      `flux-lang`/`flux-flow`/`flux-eval` tests still pass byte-identically**.
- [x] Dotted identifier access — a formula referencing `it.a.b` (or any declared var with
...
- [x] New expr builtins (registered in `expr_call_fn` **and** `is_known_expr_fn`):
...
- [x] `ExprVal::Obj` truthy iff non-empty, `as_text` renders compact JSON, `len(obj)`
      returns key count. Failing-first test: `expr_val_obj_truthiness_and_render`.
- [x] Analyzer's `validate_expr_formula` accepts the new builtins automatically (shared
      whitelist); an unknown function still errors with the existing diagnostic.
- [x] SSOT regenerated: `Node::Expr` doc-comment reflects the extended surface; `UPDATE=1`
      run of `skill_in_sync` / `website_in_sync` / `skill_docs_in_sync` produces zero
      further diff.
- [x] CHANGELOG entry under `[Unreleased]`.

## Progress
- Done 2026-07-09: `flux_lang::expr` is public; runtime/analyzer share it; dotted access, object
  preservation, list builtins, docs, changelog, and sync gates are in place.
