...
## Acceptance
- [ ] All 19 plugin `manifest_builder`s report `env!("CARGO_PKG_VERSION")`; no hardcoded manifest
      version strings remain in `plugins/*/src/main.rs`.
- [ ] Plugins workspace bumped to 0.1.1; full workspace tests + fmt + clippy green.
- [ ] `release-plugins` workflow dispatched with `version=0.1.1, publish=true` (never a hand-pushed
      tag); the signed `plugins-v0.1.1` release exists with 19 plugins × 5 targets + signed index.
- [ ] Post-release: `flux plugin install slack` resolves 0.1.1, `plugin status slack` shows
