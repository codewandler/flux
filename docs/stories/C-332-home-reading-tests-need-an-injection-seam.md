...
- [x] **The three follow-ups C-297 recorded become real, ranked stories** (or are closed here with
      ...
      → re-derived below; filed as [C-391](C-391-worktree-tests-write-into-the-operators-home.md),
      [C-392](C-392-server-router-tests-read-the-operators-config.md),
      [C-393](C-393-detect-signals-has-no-injection-seam.md).
...
- [x] ⚠ **~20 `flux-tools` tests create and delete real directories under the developer's
      ...
      real `~/.flux/worktrees`. Filed as C-391 at the top rank; not half-fixed here, because the
...
| `flux-server` router | `router_in` | 25 | **43** | filed → C-392 |
| `flux-flow` engine turns | `detect_signals_in` | 4 | **4** | filed → C-393 |
| `flux-runtime` direct | `detect_signals_in` | 2 | **3** | filed → C-393 |
| `flux-app` | `detect_signals_in` | 1 | **1** | filed → C-393 |
| `flux-tools` worktree | worktree base dir | 6 | **5** (+3 that refuse before allocating) | filed → C-391 |
| `flux-agent` skill discovery | `DiscoveryEnv` | 2 | **2** | filed → C-393 |
| `flux-sdk` skill discovery | `DiscoveryEnv` | — | **1** | filed → C-393 |
...
exercises process `HOME`. Recorded in C-392's Notes.
...
- ⚠ **Trap for a resuming agent.** `crates/flux-config/src/lib.rs`'s `HOME_LOCK` is now held by
  exactly three tests. If C-392 removes the tilde-expansion dependency, delete the lock in the same
