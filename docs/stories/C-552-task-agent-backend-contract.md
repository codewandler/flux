...
depends_on: [C-542, C-569, C-570, C-575]
note: "follow-up after native loop/report contracts — lifecycle stays generic and advertises which harness and budget semantics a backend can uphold"
---
...
- [ ] The port covers capability discovery, start, accepted/delivered/completed steering, status,
      cancellation, resume and terminal result receipts with stable worker/session ids. Start accepts
      C-569's resolved loop binding, C-542's budget envelope and C-570's report channel.
- [ ] Discovery declares arbitrary-native-loop versus named-backend-profile support, exact resume/
      entry-point semantics, acknowledged progress/yield and each metered/enforced budget dimension.
      Unsupported combinations refuse before dispatch rather than falling back to backend defaults.
...
- [ ] Progress, yield, usage settlement and budget exhaustion map to typed durable receipts; stdout
      prose and process liveness never substitute for them.
- [ ] Resource discovery names which C-575 CPU/network/byte/process dimensions are measured,
      enforced, observed-only or unsupported; absent foreign-harness telemetry never becomes zero.
