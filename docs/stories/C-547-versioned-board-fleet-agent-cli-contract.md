...
- [x] Failing-first CLI tests pin the `flux.cli/v1` success and failure envelopes, deterministic
      ordering and clean separation of JSON stdout from diagnostics; the commands do not exist at
      the merge base.
- [x] Shared CLI plumbing supports `--output human|json|ndjson`, `--request FILE|-`,
      `--idempotency-key`, `--if-revision` and `--dry-run`; unsupported combinations return typed
      schema errors rather than being ignored.
- [x] Exit classes distinguish input/schema, not-found, conflict/precondition, permission,
      transient worker and validation/gate failures. Public tests pin both code and error body.
- [x] A durable idempotency record makes a repeated mutating request return its original result, and
      optimistic revision tests prove a stale caller cannot overwrite a newer board/fleet state.
- [x] `board call`/`fleet call` and `board schema`/`fleet schema` share the exact operation schemas
      used by ergonomic commands; schema fixtures fail when a public command drifts.
- [x] `board skill` and `fleet skill` use concise in-tree templates, render valid Agent Skill
...
- [x] Sensitive fields and paths are redacted before entering any human, JSON or NDJSON result.
- [x] Website automation documentation and CLI help state that JSON, not human prose, is the agent
      API; user-visible release notes are updated.
- [x] Targeted CLI and Markdown/skill tests pass; the final board wave owns the full repository gate.
