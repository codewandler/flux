...
- [ ] Before implementation, one documented contract is chosen and used consistently by
...
- [ ] The chosen contract prevents a concurrent, contract-compliant Flux build or cleanup from
...
- [ ] `CARGO_TARGET_DIR` remains an explicit operator override and is resolved exactly once using
...
- [ ] Ordinary `cargo build|test|clippy` output remains reusable across compatible invocations. The
...
- [ ] `task install` still runs the workspace library-test verification and installs both supported
      ...
      installed binaries.
- [ ] Success, Cargo failure, native compiler failure, cancellation and signal/abnormal child exit
...
- [ ] Diagnostics name the governed target, whether ownership was acquired, waited on or refused,
...
- [ ] A failing-first regression harness reproduces the old disappearance class against the
...
- [ ] Linux CI includes a real bundled-SQLite/native-compiler proof that exercises
...
- [ ] The path, lease and cleanup implementation is portable: Linux uses the real SQLite/`cc`
...
- [ ] `Taskfile.yaml`, contributor/install guidance and fleet/shared-target documentation describe
...
- [ ] The focused install/ownership tests, deterministic concurrent regression and Linux bundled-
