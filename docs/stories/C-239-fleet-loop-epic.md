...
note: "EPIC — the model reasons, the host enforces: a WaveCoordinator does isolation/gate/merge/revert/ledger so the loop's invariants hold even when the model is wrong"
---
...
an isolated worker that implements and gates and commits on a scratch branch, review the diff *as
evidence*, up to two rework rounds to the **same** worker, park after that, integrate serially with a
full gate after every merge and revert on red, then write the bookkeeping.
...
actions; the model owns wave selection and diff review. The invariants — fenced ledger, gate after
every merge, never implement, revert on red, park after two rounds — become host behaviour instead of
prose, so they hold even when the model is wrong or lazy.
...
- [ ] These are true *mechanically*, each pinned by a test rather than asserted in prose:
      integrating without gating is impossible · a red gate leaves the pre-merge tree via
      `revert -m 1` · a third rework round parks instead of dispatching · a worker cannot write a
      fenced ledger path · a `Failed→Ready` retry leaves no stale `runner`/`task_id`.
...
- 2026-07-30 — **design done**: [fleet-loop.md](../designs/fleet-loop.md), with the ten stories filed
  ...
  sharpest instance is `fleet.integrate`, which gates and merges or does neither, so
  "gate after every merge" stops being an instruction.
