...
- [x] Add a failing-first test in `flux-flow` whose provider emits at least two independent,
...
- [x] Model-stage gather and adaptive exploration use one shared native-call batch executor rather
...
- [x] Batch admission classifies every call before execution. Calls may overlap only when their
...
- [x] The provider transcript remains valid and deterministic: one assistant message containing N
...
- [x] A sibling failure is returned in its own ordered result slot and does not discard or
...
- [x] Every call retains the same immutable turn/session identity, runtime context, redactor,
...
- [x] `ResourceLimits::max_concurrent_tool_calls` remains the authoritative execution ceiling. A
...
- [x] A regression test proves effectful/conflicting calls retain the existing ordering and failure
...
- [x] The planner/native-tool guidance tells capable providers to emit independent gather calls in
      one response when useful; it does not tell them to parallelize writes or approval-sensitive
      work.
- [x] Targeted tests and the full repository gate are green. Before the pull request,
...
- 2026-08-04: the focused `codewandler-flux-flow` tests and clippy, generated website changelog
  check, complete repository gate, website documentation test, and embedded-docs check are green.
  The first embedded-docs regeneration was byte-identical; publication regeneration after story
  closure is recorded with the final board state.
