...
## Acceptance
- [ ] Failing-first: a test that deterministically reproduces the race — e.g. one that mutates
...
- [ ] Every fixture root in `crates/flux-system/src/lib.rs`'s test module is constructed through one
      guarded helper. No bare `std::env::temp_dir()` remains in the test module.
- [ ] A guard prevents regression: a test (or a clippy/codegate rule) fails if a new bare
...
- [ ] `cargo test -p codewandler-flux-system --lib` passes **20 consecutive runs** with no failure.
