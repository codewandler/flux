...
- [x] ⚠ **`crates/flux-events/src/otel.rs`'s `redact_attr` passes numeric span attributes through
      ...
      [C-344](C-344-otel-metrics-attributes-skip-the-redactor.md) because closing it changes a
...
**Filed onward as [C-344](C-344-otel-metrics-attributes-skip-the-redactor.md), not closed here**,
...
What this story *did* do so the gap cannot hide while C-344 is open: narrowed `redact_attr`'s scope
...
- Filed [C-344](C-344-otel-metrics-attributes-skip-the-redactor.md) with the probe evidence and the
  published-API cost.
