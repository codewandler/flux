...
- Closed 2026-08-02 before the first live JWT was used. `MediaSettings` now mirrors
  `WebhookSettings`: program name and argument count remain visible, every argument after argv zero
  renders only as `<redacted>`. The failing-first test covers both `MediaSettings` directly and its
  nested rendering through `RoomSettings`.
- `ChannelDecl` remains intentionally out of this local fix: it is L0's generic opaque declaration bag
  for agents, channels and datasources, and `resolve_secrets` mutates that entire `Program` in place.
  Making its `Debug` safe requires a program-wide secret-aware value wrapper or discarding resolved
  declarations after typed binding; hiding every `settings` value would make all non-secret declarations
  undiagnosable. That pre-existing architecture gap is broader than media and should be solved once at
  the declaration/resolution seam, not with a media-key heuristic in `flux-lang`.
