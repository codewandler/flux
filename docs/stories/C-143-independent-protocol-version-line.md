...
- [ ] `codewandler-flux-plugin-protocol` starts at `1.0.0` with its own `version` key (not
...
- [ ] `plugins/host-kit` versions on the protocol line (`1.x`) and depends on `^1`; the lockstep
      comment block in `plugins/host-kit/Cargo.toml` is deleted.
- [ ] `plugins/Cargo.toml`'s `[workspace.dependencies]` use caret requirements on the protocol
      line, keeping path deps for local development.
- [ ] `scripts/cut-release.sh` no longer: `sed`s `plugins/Cargo.toml` or
...
- [ ] AGENTS.md records the documented exception to the single-version rule and why C-146's
