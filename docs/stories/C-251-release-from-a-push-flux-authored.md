...
## Progress
- 2026-08-05 — release run `31038895447` again completed the provider-free cut. Promotion then
  stopped after staging only the cut ref because the configured PAT lacks Pull Request API scope.
  Because GitHub places Actions-created PR checks behind manual approval, the controller instead
  dispatches complete CI on the exact cut SHA and lets the PAT push only the reconstructed
  fast-forward two-parent merge plus later refs/tags. The failed run created no PR, candidate or tag.
- 2026-08-05 — release run `31035954598` completed the provider-free build and deterministic cut,
  ...
  patch against live main before staging a candidate. The failed run created no cut ref,
