...
- [ ] `flux-release-candidate-v3` retains the exact version, lowercase 40-hex commit SHA and positive
...
- [ ] Each record binds the API-reported artifact `name`, positive immutable database `id`, positive
...
- [ ] The narrow `flux-release-promoter` job creates `release-candidates/<tag>` only after the cut PR
...
- [ ] Promotion downloads each archive by its receipt-bound immutable artifact ID. Before opening or
...
- [ ] Each verified ZIP extracts into a fresh directory named only from its receipt record. Safe
...
- [ ] Failing-first fixtures cover missing/extra/expired artifacts; duplicate name or ID; wrong
...
- [ ] Workflow-order tests prove the merged-main-SHA check, v3 verification and safe extraction
...
- [ ] BUILD-ONCE publication documentation names receipt v3 and its raw-ZIP trust boundary. Focused
