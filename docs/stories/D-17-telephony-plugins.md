---
id: D-17
title: Telephony plugin pack (asterisk, homer)
pillar: Agent
status: done
design: docs/designs/fluxplane-plugins-parity.md
---

# Telephony plugin pack

## Goal
The two telephony integrations from fluxplane — Asterisk PBX control and Homer SIP capture. These serve the
downstream **voice** side (a managed-agents operator inspecting/controlling calls), so they complete parity in
a direction flux otherwise has no coverage for.

## Acceptance (per plugin — match the fluxplane manifest's op set)
- [x] **asterisk** (8): `ami.ping`, `command`, `peer.list`, `channel.list/hangup`, `devicestate.list`,
      `queue.status`, `call.originate` over the AMI TCP socket (D-12 `conn.dial` kind=tcp; AMI is line-based).
- [x] **homer** (8, HTTP): `test`, `search`, `call.list/show/qos/analyze`, `pcap.export`, `alias.list`
      (`config` auth via D-12 header).
- [x] `channel.hangup`/`call.originate` declared `Risk::Destructive`/write; everything else read-only except
      `asterisk.command`, which is high-risk/write.
- [x] Each: full gate green in `plugins/`; smoke entry; `flux plugin skill refresh`.

## Progress
- **Done (2026-06-30).** Ported asterisk AMI over `ConnStream` and Homer over HTTP/JWT login. Full
  `plugins/` workspace test/clippy gate is green.

## Notes
- Op shapes from `~/projects/fluxplane/fluxplane-plugins/{asterisk,homer}/manifest.go`. Serves downstream
  voice surfaces (see the realtime-voice track). Epic:
  [fluxplane-plugins-parity.md](../designs/fluxplane-plugins-parity.md).
