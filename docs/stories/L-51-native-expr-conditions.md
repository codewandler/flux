...
## Acceptance
- [ ] Parser: in the four condition positions (`when`, `unless`, `until`, `assert`) and
...
- [ ] Formatter: `format` renders `Expr` natively **only when the lowering is invertible**
      ...
      formulas.
- [ ] Analyzer: op params whose schema carries `"format": "flux-expr"` (the `where` /
...
- [ ] No grammar ambiguity: any operator tail in a condition/bind-RHS position was an
...
- [ ] SSOT regen: `syntax.md` gets a native-conditions section; `flows-and-syntax.md` /
