...
## Acceptance
- [ ] A test iterating **every** registered `ToolSpec` asserts the metadata combination is coherent.
...
- [ ] The exact invariant set is agreed and written down in the test as prose before it is coded —
      an invariant nobody can state is an invariant nobody can enforce.
- [ ] Failing-first proof: a deliberately mis-declared spec (mutating effect, `Risk::Low`) fails the
      test.
- [ ] The test covers plugin-supplied specs at registration, not only built-in ops — plugins declare
      the same metadata and are trusted the same way.
- [ ] Any legitimate exception is an explicit allowlist entry with a comment, not a loosened rule.
