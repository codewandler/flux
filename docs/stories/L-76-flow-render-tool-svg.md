...
## Acceptance
- [ ] NEW `crates/flux-tools/src/render.rs`: `pub enum View { Source, Tree }`,
...
- [ ] Theme ported **verbatim** from `flux-tree-sitter/scripts/render-example.mjs` (BG `#282c34`,
  keyword `#c678dd`, string `#98c379`, … — full table in the design) so the source view matches the
  existing README image.
- [ ] Failing-first pure-core tests: `render_flux_svg(snippet, Source)` output starts with `<svg`,
...
- [ ] Correctness notes covered by tests: a `"""` multi-line string colours on **every** line
...
- [ ] Robustness: malformed source in `Source` view still renders (no panic); in `Tree` view the
...
- [ ] Tool input mirrors `FlowRunInput`: `source` **xor** `name` (stored flow resolved by reusing
...
- [ ] Tool test with the established `ToolContext` pattern (`transform.rs:605` `ctx()`): `flow_render`
