...
- **The fence is structural, not a plea to the model.** The payload renders as one line of JSON;
  `serde_json` escapes every control character in **keys and values alike**, so no payload byte can
  emit a newline, and a marker that must occupy a line of its own cannot be forged from inside.
  Worth noting: at the merge base the *keys* were not escaped at all (`format!("{k}={v}")` printed
  a key's raw newlines), so a hostile key could already break the one-line shape — the merge-base
  run of `a_payload_value_cannot_forge_the_event_data_fence` shows it.
